  ...
  private initFloatingUI(): void {
    try {
      ...
      const onMouseUp = () => {
        ...
        // Persist position
        const rect = overlay.getBoundingClientRect();
        chrome.storage.local.set({ tbvOverlayPosition: { top: rect.top, left: rect.left } });
      };
      ...
      hideBtn.addEventListener('click', () => {
        overlay.style.display = 'none';
        chrome.storage.local.set({ tbvOverlayVisible: false });
      });
      ...
      const toggleOverlay = (show?: boolean) => {
        ...
        chrome.storage.local.set({ tbvOverlayVisible: next });
        if (next) {
          chrome.storage.local.get(['tbvOverlayPosition', 'tbvOverlaySize']).then((res) => {
      ...
      chrome.storage.local.get(['tbvOverlayVisible', 'tbvOverlayPosition', 'tbvOverlaySize']).then((res) => {
        if (res.tbvOverlayVisible === false) {
          overlay.style.display = 'none';
        }
        const pos = res.tbvOverlayPosition as { top?: number; left?: number } | undefined;
        if (pos && typeof pos.top === 'number' && typeof pos.left === 'number') {
      ...
      const onResizeUp = () => {
        ...
        chrome.storage.local.set({ tbvOverlaySize: { height: overlay.offsetHeight } });
      };
