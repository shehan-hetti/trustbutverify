...
function containmentScore(a: string, b: string): number {
  ...
  if (bSoft.includes(aSoft)) return Math.min(1, aSoft.length / bSoft.length);
  if (aSoft.includes(bSoft)) return Math.min(1, bSoft.length / aSoft.length);

  const aHard = normalizeForMatch(a, 'hard');
  const bHard = normalizeForMatch(b, 'hard');
  if (aHard && bHard) {
    if (bHard.includes(aHard)) return Math.min(1, aHard.length / bHard.length);
    if (aHard.includes(bHard)) return Math.min(1, bHard.length / aHard.length);
  }
...
function extractCategoryAndSummary(content: string): { category: string; summary: string } | null {
  ...
  if (json && typeof json === 'object') {
    ...
    if (category && summary) return { category, summary };
  }
...
describe('extractCategoryAndSummary', () => {
  it('extracts from valid JSON', () => {
    const result = extractCategoryAndSummary(
      '{"category":"Debugging|Python","summary":"User wants to fix a bug in their code."}'
    );
  ...
  it('handles category as array', () => {
    const result = extractCategoryAndSummary(
      '{"category":["Debugging","Python"],"summary":"Bug fix."}'
    );
