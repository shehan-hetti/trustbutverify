...
/*  Helpers                                                            */
/* ------------------------------------------------------------------ */
function makeCopyActivity(overrides) {
    return {
...
describe('StorageManager – conversations & turns', () => {
    ...
    it('upsertConversationTurns creates a new conversation', async () => {
        const turn = makeTurn();
        await StorageManager.upsertConversationTurns('chatgpt.com::thread-1', { url: 'https://chatgpt.com/c/thread-1', domain: 'chatgpt.com', platform: 'ChatGPT' }, [turn]);
        const convos = await StorageManager.getAllConversations();
...
describe('StorageManager – copy activities', () => {
    ...
    it('getRecentActivities respects the limit', async () => {
        for (let i = 0; i < 5; i++) {
            await StorageManager.saveActivity(makeCopyActivity({ id: `a-${i}`, timestamp: Date.now() + i, conversationId: 'chatgpt.com::t1' }));
        }
    ...
    it('patchCopyActivityById updates specific fields', async () => {
        await StorageManager.saveActivity(makeCopyActivity({ id: 'patch-me', conversationId: 'chatgpt.com::t1', copyCategory: undefined }));
...
describe('StorageManager – nudge events', () => {
    ...
    it('getNudgeAggregateStats computes correct totals', async () => {
        await StorageManager.saveNudgeEvent(makeNudgeEvent({ id: 'ne-a', triggerType: 'copy', response: 'yes' }));
        await StorageManager.saveNudgeEvent(makeNudgeEvent({ id: 'ne-b', triggerType: 'copy', response: 'skip' }));
        await StorageManager.saveNudgeEvent(makeNudgeEvent({ id: 'ne-c', triggerType: 'response', response: 'no' }));
...
describe('StorageManager – getStorageStats', () => {
    ...
    it('computes stats correctly with data', async () => {
        await StorageManager.upsertConversationTurns('chatgpt.com::t1', { url: 'https://chatgpt.com/c/t1', domain: 'chatgpt.com' }, [
            makeTurn({
                responseTimeMs: 2000,
        ...
        await StorageManager.saveActivity(makeCopyActivity({ conversationId: 'chatgpt.com::t1' }));
        const stats = await StorageManager.getStorageStats();
